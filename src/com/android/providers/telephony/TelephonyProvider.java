...
import static android.provider.Telephony.Carriers.TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS;
...
import static android.provider.Telephony.Carriers.MTU_IN_BYTES;
...
import static android.provider.Telephony.Carriers.NO_ID_SET;
...
import static android.provider.Telephony.Carriers.WAIT_TIME_RETRY_IN_MS;
...
public class TelephonyProvider extends ContentProvider
{
    ...
    static {
        ...
        CARRIERS_UNIQUE_FIELDS_DEFAULTS.put(APN_SET_ID, String.valueOf(NO_ID_SET));
    ...
    public static String getStringForCarrierTableCreation(String tableName) {
        return "CREATE TABLE " + tableName +
                ...
                WAIT_TIME_RETRY_IN_MS + " INTEGER DEFAULT 0," +
                TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS + " INTEGER DEFAULT 0," +
                MTU_IN_BYTES + " INTEGER DEFAULT 0," +
                EDITED_STATUS + " INTEGER DEFAULT " + UNEDITED + "," +
                USER_VISIBLE + " BOOLEAN DEFAULT 1," +
                USER_EDITABLE + " BOOLEAN DEFAULT 1," +
                OWNED_BY + " INTEGER DEFAULT " + OWNED_BY_OTHERS + "," +
                APN_SET_ID + " INTEGER DEFAULT " + NO_ID_SET + "," +
    ...
    public static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private void deleteRow(SQLiteDatabase db, ContentValues values) {
            ...
            String where = queryValFirst(NUMERIC) +
                    ...
                    queryValOrNull(WAIT_TIME_RETRY_IN_MS) +
                    queryValOrNull(TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS) +
                    queryValOrNull(MTU_IN_BYTES);
            ...
            whereArgs[i++] = values.containsKey(WAIT_TIME_RETRY_IN_MS) ?
                    values.getAsString(WAIT_TIME_RETRY_IN_MS) : "0";
            whereArgs[i++] = values.containsKey(TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS) ?
                    values.getAsString(TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS) : "0";
            whereArgs[i++] = values.containsKey(MTU_IN_BYTES) ?
                    values.getAsString(MTU_IN_BYTES) : "0";
        ...
        private void copyApnValuesV17(ContentValues cv, Cursor c) {
            ...
            getIntValueFromCursor(cv, c, WAIT_TIME_RETRY_IN_MS);
            getIntValueFromCursor(cv, c, TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS);
            getIntValueFromCursor(cv, c, MTU_IN_BYTES);
        ...
        private ContentValues getRow(XmlPullParser parser) {
            ...
            addIntAttribute(parser, "wait_time", map, WAIT_TIME_RETRY_IN_MS);
            addIntAttribute(parser, "max_conns_time", map, TIME_LIMIT_FOR_MAX_CONNECTIONS_IN_SECONDS);
            addIntAttribute(parser, "mtu", map, MTU_IN_BYTES);
