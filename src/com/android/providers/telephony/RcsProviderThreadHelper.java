...
class RcsProviderThreadHelper {
    ...
    public static void createThreadTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE VIEW " + UNIFIED_RCS_THREAD_VIEW
                ...
                + "null AS group_name, null AS group_icon, null AS conference_uri, 0 AS is_group "
                + "FROM rcs_1_to_1_thread UNION SELECT rcs_thread_id, null AS "
                + "rcs_fallback_thread_id, owner_participant, group_name, group_icon, "
                + "conference_uri, 1 AS is_group FROM rcs_group_thread");
    ...
    long insert1To1Thread(ContentValues contentValues) {
        ...
        if (contentValues.containsKey(ID_COLUMN) || contentValues.containsKey(
    ...
    long insertGroupThread(ContentValues contentValues) {
        ...
        try {
            ...
            contentValues.put(RCS_THREAD_ID_COLUMN, rowId);
            long temp = db.insert(RCS_GROUP_THREAD_TABLE, RCS_THREAD_ID_COLUMN, contentValues);
            contentValues.remove(RCS_THREAD_ID_COLUMN);
    ...
    private long insertIntoCommonRcsThreads(SQLiteDatabase db) {
        return db.insert(THREAD_TABLE, ID_COLUMN, new ContentValues());
    }
    ...
    int deleteUnifiedThread(String selection, String[] selectionArgs) {
        int deletedCount;
        SQLiteDatabase db = mSqLiteOpenHelper.getWritableDatabase();
        Cursor cursor = queryUnifiedThread(new String[]{IS_GROUP},
