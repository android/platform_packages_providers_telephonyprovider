...
import static android.provider.Telephony.RcsColumns.RcsFileTransferColumns.LENGTH_COLUMN;
...
import static android.provider.Telephony.RcsColumns.RcsFileTransferColumns.TRANSFER_OFFSET_COLUMN;
...
import static android.provider.Telephony.RcsColumns.RcsIncomingMessageColumns.NOTIFIED_TIMESTAMP_COLUMN;
import static android.provider.Telephony.RcsColumns.RcsIncomingMessageColumns.SENDER_PARTICIPANT_COLUMN;
...
import static android.provider.Telephony.RcsColumns.RcsMessageColumns.TEXT_COLUMN;
import static android.provider.Telephony.RcsColumns.RcsMessageDeliveryColumns.DELIVERED_TIMESTAMP_COLUMN;
import static android.provider.Telephony.RcsColumns.RcsMessageDeliveryColumns.SEEN_TIMESTAMP_COLUMN;
...
import static android.provider.Telephony.RcsColumns.RcsUnifiedMessageColumns.IS_INCOMING_COLUMN;
...
import static android.telephony.ims.RcsMessageQueryContinuationToken.MESSAGE_QUERY_CONTINUATION_TOKEN;
import static android.telephony.ims.RcsMessageQueryParameters.INCOMING_AND_OUTGOING;
import static android.telephony.ims.RcsMessageQueryParameters.INCOMING_ONLY;
import static android.telephony.ims.RcsMessageQueryParameters.MESSAGE_QUERY_PARAMETERS_KEY;
import static android.telephony.ims.RcsMessageQueryParameters.OUTGOING_ONLY;
...
public class RcsProviderMessageHelper {
    ...
    public static void createRcsMessageTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + RCS_MESSAGE_TABLE + "(" + MESSAGE_ID_COLUMN
                ...
                + GLOBAL_ID_COLUMN + " TEXT, " + SUB_ID_COLUMN + " INTEGER, " + TEXT_COLUMN
        ...
        db.execSQL("CREATE TABLE " + RCS_INCOMING_MESSAGE_TABLE + "(" + MESSAGE_ID_COLUMN
                ...
                + ARRIVAL_TIMESTAMP_COLUMN + " INTEGER, " + NOTIFIED_TIMESTAMP_COLUMN
                + " INTEGER, FOREIGN KEY (" + MESSAGE_ID_COLUMN + ") REFERENCES "
        ...
        db.execSQL("CREATE TABLE " + RCS_MESSAGE_DELIVERY_TABLE + "(" + MESSAGE_ID_COLUMN
                ...
                + DELIVERED_TIMESTAMP_COLUMN + " INTEGER, " + SEEN_TIMESTAMP_COLUMN + " INTEGER, "
        ...
        db.execSQL("CREATE TABLE " + RCS_FILE_TRANSFER_TABLE + " (" + FILE_TRANSFER_ID_COLUMN
                ...
                + TRANSFER_OFFSET_COLUMN + " INTEGER, " + TRANSFER_STATUS_COLUMN + " INTEGER, "
                + WIDTH_COLUMN + " INTEGER, " + HEIGHT_COLUMN + " INTEGER, " + LENGTH_COLUMN
                + " INTEGER, " + PREVIEW_URI_COLUMN + " TEXT, " + PREVIEW_TYPE_COLUMN +
                " TEXT, FOREIGN KEY (" + MESSAGE_ID_COLUMN + ") REFERENCES " + RCS_MESSAGE_TABLE
                + "(" + MESSAGE_ID_COLUMN + "))");
        ...
        //
        db.execSQL(
                "CREATE VIEW " + UNIFIED_MESSAGE_VIEW + " AS SELECT "
                        + "rcs_message.rcs_message_row_id, rcs_message.rcs_thread_id, "
                        + "rcs_message.rcs_message_global_id, rcs_message.sub_id, "
                        + "rcs_message.status, rcs_message.origination_timestamp, "
                        + "rcs_message.rcs_text, 0 AS sender_participant, 0 AS arrival_timestamp, 0"
                        + " AS notified_timestamp, 0 AS is_incoming FROM rcs_message INNER JOIN "
                        + "rcs_outgoing_message ON "
                        + "rcs_message.rcs_message_row_id=rcs_outgoing_message.rcs_message_row_id "
                        + "UNION SELECT rcs_message.rcs_message_row_id, rcs_message.rcs_thread_id, "
                        + "rcs_message.rcs_message_global_id, rcs_message.sub_id, "
                        + "rcs_message.status, rcs_message.origination_timestamp, "
                        + "rcs_message.rcs_text, rcs_incoming_message.sender_participant, "
                        + "rcs_incoming_message.arrival_timestamp, "
                        + "rcs_incoming_message.notified_timestamp, 1 AS is_incoming FROM "
                        + "rcs_message INNER JOIN rcs_incoming_message ON "
                        + "rcs_message.rcs_message_row_id=rcs_incoming_message.rcs_message_row_id");
    ...
    private Cursor performInitialQuery(RcsMessageQueryParameters queryParameters) {
        ...
        @RcsMessageQueryParameters.MessageType int messageType = queryParameters.getMessageType();
        ...
        if (limit > 0) {
            RcsProviderUtil.createContinuationTokenBundle(cursor,
                    new RcsMessageQueryContinuationToken(rawQueryAsString, limit, limit),
    ...
    private ContentValues getIncomingMessageValues(ContentValues allValues) {
        ...
        if (allValues.containsKey(NOTIFIED_TIMESTAMP_COLUMN)) {
            incomingMessageValues.put(
                    NOTIFIED_TIMESTAMP_COLUMN, allValues.getAsLong(NOTIFIED_TIMESTAMP_COLUMN));
