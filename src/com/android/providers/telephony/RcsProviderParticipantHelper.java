...
public class RcsProviderParticipantHelper {
    ...
    static void insert(SQLiteDatabase db, ContentValues values) {
        if (!values.containsKey(RcsMessageStoreController.PARTICIPANT_ADDRESS_KEY)) {
            Log.e(TAG, "Inserting a participant without address is not supported.");
            return;
        }

        int canonicalAddressId = getCanonicalAddressId(db, values);
        if (canonicalAddressId == NO_EXISTING_ADDRESS) {
            return;
        }

        ContentValues rcsParticipantContentValues = new ContentValues();
        rcsParticipantContentValues.put(CANONICAL_ADDRESS_ID_COLUMN, canonicalAddressId);

        db.insert(PARTICIPANT_TABLE, ID_COLUMN, values);
