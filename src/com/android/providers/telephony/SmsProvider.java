...
public class SmsProvider extends ContentProvider {
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        switch (match) {
            ...
            case SMS_ICC:
                {
                    boolean deleteMultiple = whereArgs != null && whereArgs.length > 0;
                    String[] indexString =
                            deleteMultiple
                                    ? whereArgs
                                    : new String[] {url.getPathSegments().get(1)};
                    count =
            ...
            case SMS_ICC_SUB:
                {
                    ...
                    count =
                            deleteMessageFromIcc(
                                    Integer.parseInt(subId), deleteMultiple, indexString);
                }
                break;
    ...
    private int deleteMessageFromIcc(int subId, String messageIndex) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Bad SMS ICC ID: " + messageIndex);
        } finally {
            Binder.restoreCallingIdentity(token);
        }
    }
            ...
            for (; count < messageIndexString.length; count++) {
                boolean result =
                        smsManager.deleteMessageFromIcc(
                                Integer.parseInt(messageIndexString[count]));
                if (!result) { // If delete operation failed
                    deleteAllSuccess = false;
                }
            }
            return deleteAllSuccess ? count : 0;
        } catch (NumberFormatException exception) {
