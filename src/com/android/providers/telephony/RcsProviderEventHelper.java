...
import static android.provider.Telephony.RcsColumns.RcsParticipantEventColumns.OLD_ALIAS_COLUMN;
import static android.provider.Telephony.RcsColumns.RcsParticipantEventTypes.ICON_CHANGED_EVENT_TYPE;
import static android.provider.Telephony.RcsColumns.RcsParticipantEventTypes.NAME_CHANGED_EVENT_TYPE;
import static android.provider.Telephony.RcsColumns.RcsParticipantEventTypes.PARTICIPANT_JOINED_EVENT_TYPE;
import static android.provider.Telephony.RcsColumns.RcsParticipantEventTypes.PARTICIPANT_LEFT_EVENT_TYPE;
...
import static android.provider.Telephony.RcsColumns.RcsThreadEventColumns.OLD_ICON_URI_COLUMN;
...
import static android.telephony.ims.RcsEventQueryContinuationToken.EVENT_QUERY_CONTINUATION_TOKEN;
...
class RcsProviderEventHelper {
    ...
    public static void createRcsEventTables(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + RCS_THREAD_EVENT_TABLE + "(" + EVENT_ID_COLUMN
                ...
                + ORIGINATING_PARTICIPANT_COLUMN + " INTEGER, " + EVENT_TYPE_COLUMN + " INTEGER, "
                + TIMESTAMP_COLUMN + " INTEGER, " + DESTINATION_PARTICIPANT_COLUMN + " INTEGER, "
                + OLD_ICON_URI_COLUMN + " TEXT, " + NEW_ICON_URI_COLUMN + " TEXT, "
                + OLD_NAME_COLUMN + " TEXT, " + NEW_NAME_COLUMN + " TEXT, " + " FOREIGN KEY ("
                + RCS_THREAD_ID_COLUMN + ") REFERENCES " + RCS_THREAD_TABLE + " ("
                + RCS_THREAD_ID_COLUMN + "), FOREIGN KEY (" + ORIGINATING_PARTICIPANT_COLUMN
        ...
        db.execSQL("CREATE TABLE " + RCS_PARTICIPANT_EVENT_TABLE + "(" + EVENT_ID_COLUMN
                + " INTEGER PRIMARY KEY AUTOINCREMENT, " + ORIGINATING_PARTICIPANT_COLUMN +
                " INTEGER, " + TIMESTAMP_COLUMN + " INTEGER, " + OLD_ALIAS_COLUMN + " TEXT, " +
                NEW_ALIAS_COLUMN + " TEXT," + " FOREIGN KEY (" + ORIGINATING_PARTICIPANT_COLUMN
                + ") REFERENCES " + RCS_PARTICIPANT_TABLE + " (" + RCS_PARTICIPANT_ID_COLUMN
                + "))");
    ...
    private Cursor performInitialQuery(RcsEventQueryParameters queryParameters) {
        ...
        if (limit > 0) {
            RcsProviderUtil.createContinuationTokenBundle(cursor,
                    new RcsEventQueryContinuationToken(rawQueryAsString, limit, limit),
    ...
    private long insertParticipantChangedEvent(Uri uri, ContentValues valuesParameter,
            int eventType) {
