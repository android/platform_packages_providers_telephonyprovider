...
public final class TelephonyDatabaseHelperTest {
    ...
    @Test
    public void databaseHelperOnUpgrade_hasD2DSharingStatusField() {
        Log.d(TAG, "databaseHelperOnUpgrade_hasD2DSharingStatusField");
        // (5 << 16 | 6) is the first upgrade trigger in onUpgrade
        SQLiteDatabase db = mInMemoryDbHelper.getWritableDatabase();
        mHelper.onUpgrade(db, (4 << 16), TelephonyProvider.getVersion(mContext));
        ...
        assertTrue(Arrays.asList(upgradedColumns).contains(
                Telephony.SimInfo.COLUMN_D2D_SHARING_STATUS));
    }
